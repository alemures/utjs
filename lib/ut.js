...
(function () {
  ...
  function sort(array, comparator, left, right) {
    right = isNumber(comparator) ? left : right;
    left = isNumber(comparator) ? comparator : left;

    left = left || 0;
    right = right || array.length - 1;
    comparator = isFunction(comparator) ? comparator : function (a, b) { return a - b; };
  ...
  function binaryInsert(value, array, comparator, rejectDuplicates) {
    ...
    rejectDuplicates = isBoolean(comparator) ? comparator : rejectDuplicates || false;
    ...
    while (left <= right) {
      ...
      var cmpValue = comparator ? comparator(computed, value) : undefined;

      if (comparator ? cmpValue > 0 : computed > value) {
        right = middle - 1;
        continue;
      }
