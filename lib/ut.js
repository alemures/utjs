...
(function() {
   ...
   *     ut.sort(v) = 736ms, v.sort() = 1483ms
   *   Random numbers:
   *     ut.sort(v) = 131ms, v.sort() = 324ms
   *   Ascending sorted numbers:
   *     ut.sort(v) = 85ms, v.sort() = 297ms
   *   Descending sorted numbers:
   *     ut.sort(v) = 92ms, v.sort() = 294ms
   * @param  {Array} v The array to sort.
   * @global
   */
  function sort(v) {
    var beg = [];
   ...
   * @param  {String} string     The string.
   * @param  {String} substr     The substring to be replaced as a RegExp pattern.
   *                             Use ut.escapeRegExp(String) to replace a literal string.
   * @param  {String} newSubstr  The String that replaces the substr param.
   * @param  {Boolean} [ignoreCase] If ignore case or not.
   ...
   * @param  {Object} dest   The destiny object.
   ...
   * @param  {Boolean} [strict] If strict comparation or not.
   ...
   * Ej:
   * <pre>
   *   var v = [{lang:'spanish', age: 2}, {lang:'spanish', age:5}, {lang:'english', age:25}]
   *   ut.groupBy(v, ['lang'], function(obj) { return obj.age; })
  ...
  /**
   * If a value has a numeric value or not.
   ...
   * @param  {Number}   [times] How many times to execute.
   ...
   * @param  {Number} [min] Min inclusive value.
   * @param  {Number} [max] Max inclusive value.
  ...
  /**
   * A counter to create unique function names.
   * @type {Number}
   */
  var _copyFunctionCounter = 0;

  /**
   * A regex to rename a function.
   * @type {RegExp}
   */
  var _copyFunctionRegExp = new RegExp('function\s*\w*\s*\\(');

  /**
   * Perform a full copy of the given function (THIS IS NOT A REFERENCE).
   * @param  {Function} func The function to copy.
   * @param  {String} [name] The name for the new copied function.
   * @return {Function}      The new copied function.
   * @global
   */
  function copyFunction(func, name) {
    name = name !== undefined ? name : '_copiedFunct';

    var funcName = name + (++_copyFunctionCounter);
    var funcData = func.toString().replace(_copyFunctionRegExp,
        'function ' + funcName + ' (');
    var copy = null;
  ...
  /**
   * Perform a full copy of all the elements in superCtor.prototype
   * to ctor.prototype.
   * @param  {Function} ctor      The destiny function.
   * @param  {Function} superCtor The source function.
   * @global
   */
  function copyPrototype(ctor, superCtor) {
    var superCtorProto = superCtor.prototype;
    var ctorProto = ctor.prototype;

    for (var name in superCtorProto) {
      if (isFunction(superCtorProto[name])) {
        ctorProto[name] = copyFunction(superCtorProto[name], name);
      } else {
        ctorProto[name] = superCtorProto[name];
      }
    }
  }

  /**
   * Perform a hardInherits from superCtor to ctor. This function has
   * the same behaviour than util.inherits() except it's performing a
   * full copy of all the functions instead of copy just the references.
  ...
  global.copyFunction = copyFunction;
  global.copyPrototype = copyPrototype;
  global.hardInherits = hardInherits;
