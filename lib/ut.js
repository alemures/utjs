...
(function () {
  ...
  function sort(array) {
    ...
    while (i >= 0) {
      ...
      if (left < right) {
        ...
        piv = array[left];
        while (left < right) {
          while (array[right] >= piv && left < right) { right--; }

          if (left < right) { array[left++] = array[right]; }

          while (array[left] <= piv && left < right) { left++; }
   ...
   * @global
   */
  function binaryInsert(value, array, rejectDuplicates) {
  ...
  /**
   * Returns a new array without duplicates values.
   * @param  {Array} array The array with duplications.
   * @return {Array}       New array without duplications.
   * @global
   */
  function removeDuplicates(array) {
    var array2 = [];
    for (var i = 0; i < array.length; i++) {
      binaryInsert(array[i], array2, true);
    }
